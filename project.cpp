...
bool ATM::withdraw() {
    ...
    cout << (ui->getLanguage() ? "Error: Withdrawal limit exceeded. Returning to User Menu..."
        : "오류: 최대 인출 횟수를 초과했습니다. 사용자 메뉴로 돌아갑니다...") << endl;
    cout << (ui->getLanguage() ? "Press Enter to continue..." : "계속하려면 Enter를 누르세요...");
    cin.ignore();
    cin.get();
    return false;
...
void display_history(bool exit, const vector<record*>& transaction_records, const Interface& ui) {
    if (transaction_records.empty()) {
        //////////////
        ui.showErrorUI(ui.getLanguage() ? "There is no transaction history." : "거래 내역이 없습니다.");
        ////////////////////
        return;
    }
    if (!exit) {
                    ...
                    while (true) {
                        ...
                        for (int i = 0; i < bank_list.size(); i++) {
                            if (bank_list[i]->getBankName() == bankName) {
                                cout << (ui.getLanguage() ? "The bank already exists." : "이미 존재하는 은행입니다.") << endl;
                                isSame = true;
                                break;
                            }
                        }
                    ...
                    while (true) {
                        ...
                        for (int i = 0; i < bank_list.size(); i++) {
                            if (bank_list[i]->getBankNumber() == bankCode) {
                                cout << (ui.getLanguage() ? "Bank codes cannot be duplicated." : "은행 코드는 중복될 수 없습니다.") << endl;
                                isDuplicate = true;
                                break;
                            }
                        }
