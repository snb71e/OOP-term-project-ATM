...
private:
    string tr_id; // 거래 id
    int tr_card;  // 카드번호
    ATM* atm; // ATM 객체에 대한 포인터
    static int serial_num; // 거래 시리얼 번호 (정적 변수로 모든 거래 공유)

public:
    long long int tr_money; // 입출금 금액
    int tr_fee; // 수수료
    long long int tr_total; // 수수료 포함한 거래 금액
    int tr_1000; // 1000원권 수
    int tr_5000; // 5000원권 수
    int tr_10000; // 10000원권 수
    int tr_50000; // 50000원권 수
    string session; // 세션 정보
    string session_atm; // ATM 세션
    string session_user; // 사용자 세션

    // 거래 수수료를 계산하는 함수
    int fee(int type, int money, ACCOUNT* senderAccount, ACCOUNT* receiverAccount);
...
// 트랜잭션 클래스의 시리얼 넘버 초기화
int TRANSACTION::serial_num = 0;

// 기본 생성자
TRANSACTION::TRANSACTION(ATM* a) {
    atm = a;
    serial_num += 1; // 새로운 거래가 생성될 때마다 시리얼 번호 증가
    atm->increase_session(); // ATM 세션 증가
}
...
int TRANSACTION::fee(int type, int money, ACCOUNT* senderAccount, ACCOUNT* receiverAccount) {
    switch (type) {
        ...
        case 2:
            return 5000; // 현금 계좌 이체
        case 3:
            if (senderAccount && isPrimary(atm, senderAccount) && receiverAccount && isPrimary(atm, receiverAccount)) 
                return 2000; // 주거래 간 계좌이체
            else if ((senderAccount && isPrimary(atm, senderAccount)) || (receiverAccount && isPrimary(atm, receiverAccount))) 
                return 3000; // 주거래/비주거래 간 계좌이체
            else
...
// 거래 정보를 기록하는 함수 (가상 함수, 자식 클래스에서 구현)
void TRANSACTION::write(ATM* atm) {
    //아직 못함
}
...
private:
    int tr_money_imsi; // 임시 입금 금액
    int type; // 거래 타입
    int deposit_type; // 입금 타입 (예: 현금 입금)
    string deposit_type_str; // 입금 타입 문자열 표현
    char re; // 반복 여부 (예: ATM의 버튼 입력)
    int re_count = 0; // 반복 횟수

public:
    TR_DEPOSIT(USER* user, ATM* atm, ACCOUNT* acc, CARD* card, int atm_language, int user_num);
    void write(ATM* atm) override; // 거래 내역 기록
    ~TR_DEPOSIT(); // 소멸자
};

// TR_DEPOSIT 생성자 정의
TR_DEPOSIT::TR_DEPOSIT(USER* user, ATM* atm, ACCOUNT* acc, CARD* card, int atm_language, int user_num) : TRANSACTION(atm) {
    // 입금에 관련된 추가 초기화 작업을 여기서 수행
    // 예: 입금 금액, 입금 타입 등을 사용자 입력으로 받음
    tr_money = 0; // 초기화
    tr_fee = fee(0, tr_money, acc, nullptr); // 예시로 수수료를 설정
    tr_total = tr_money + tr_fee;
    tr_1000 = tr_5000 = tr_10000 = tr_50000 = 0; // 금액권 수 초기화
    session = to_string(user_num); // 세션 정보 설정
}

void TR_DEPOSIT::write(ATM* atm) {
    // 입금 거래에 대한 정보를 기록하는 코드 구현
    cout << "Deposit Transaction Completed: " << tr_money << "원 입금\n";
    cout << "Transaction Fee: " << tr_fee << "원\n";
}
...
private:
    int type; // 거래 타입
    char re_str; // 반복 여부 (예: ATM의 버튼 입력)

public:
    TR_WITHDRAWAL(USER* user, ATM* atm, ACCOUNT* acc, CARD* card, int atm_language, int user_num);
    ~TR_WITHDRAWAL(); // 소멸자
    int inputWithdrawalAmount_eng(ACCOUNT* acc); // 출금 금액 입력 (영어)
    int inputWithdrawalAmount_kor(ACCOUNT* acc); // 출금 금액 입력 (한국어)
    bool valid(ATM* atm, ACCOUNT* acc); // 출금 유효성 검사
    void write(ATM* atm) override; // 거래 내역 기록
};

TR_WITHDRAWAL::TR_WITHDRAWAL(USER* user, ATM* atm, ACCOUNT* acc, CARD* card, int atm_language, int user_num) : TRANSACTION(atm) {
    tr_money = 0; // 초기화
    tr_fee = fee(1, tr_money, acc, nullptr); // 예시로 수수료를 설정
    tr_total = tr_money - tr_fee;
    tr_1000 = tr_5000 = tr_10000 = tr_50000 = 0; // 금액권 수 초기화
    session = to_string(user_num); // 세션 정보 설정
}
...
void TR_WITHDRAWAL::write(ATM* atm) {
    // 출금 거래에 대한 정보를 기록하는 코드 구현
    cout << "Withdrawal Transaction Completed: " << tr_money << "원 출금\n";
    cout << "Transaction Fee: " << tr_fee << "원\n";
}
...
class TR_TRANSFER : public TRANSACTION {
private:
    string give_bank; // 송금 은행
    string give_acc; // 송금 계좌
    string get_bank; // 받는 은행
    string get_acc; // 받는 계좌
    long long int acc_money; // 송금 계좌 잔액
    long long int atm_money; // ATM에 있는 현금
    int type; // 거래 타입

public:
    bool check; // 거래 유효성 검사 결과

    TR_TRANSFER(ADMIN* admin, ATM* atm, ACCOUNT* acc, USER* user, CARD* card, int atm_language, int user_num);
    bool valid(int type, int money, ATM* atm, ACCOUNT* acc); // 거래 유효성 검사
    bool in_bank(ADMIN* admin, string bank_name, string acc_num); // 은행 정보 체크
    void write(ATM* atm) override; // 거래 내역 기록
    ~TR_TRANSFER(); // 소멸자
};

TR_TRANSFER::TR_TRANSFER(ADMIN* admin, ATM* atm, ACCOUNT* acc, USER* user, CARD* card, int atm_language, int user_num) : TRANSACTION(atm) {
    // 이체 관련 추가 초기화 작업을 여기서 수행
    give_bank = "은행명"; // 예시로 설정
    give_acc = "계좌번호";
    get_bank = "받는은행명";
    get_acc = "받는계좌번호";
    tr_money = 0; // 예시로 초기화
    tr_fee = fee(2, tr_money, acc, nullptr); // 수수료 계산
    tr_total = tr_money + tr_fee;
}

bool TR_TRANSFER::valid(int type, int money, ATM* atm, ACCOUNT* acc) {
    // 이체 유효성 검사 (잔액 및 계좌 정보 등)
    return true;
}

bool TR_TRANSFER::in_bank(ADMIN* admin, string bank_name, string acc_num) {
    // 은행 및 계좌 정보 검증
    return true;
}

void TR_TRANSFER::write(ATM* atm) {
    // 이체 거래에 대한 정보를 기록하는 코드 구현
    cout << "Transfer Transaction Completed: " << tr_money << "원 이체\n";
    cout << "Transaction Fee: " << tr_fee << "원\n";
}
