...
void display_atm(vector<ATM*> atm_list, Interface ui) {
    if (atm_list.empty()) {
        ...
        for (int i = 0; i < atm_list.size(); i++) {
            cout << "ATM[SN:" << atm_list[i]->getatmID() << "] remaining cash :\nKRW 50000 : " << atm_list[i]->getremainingcash(3) << "\nKRW 10000 : " << atm_list[i]->getremainingcash(2) << "\nKRW 5000 : " << atm_list[i]->getremainingcash(1)
