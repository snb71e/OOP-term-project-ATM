...
public:
    ...
    bool withdraw() {
        ...
        while (withdrawalCount < 3) {
            ...
            // ATM 현금 부족 오류 처리
            if (!atm->hasSufficientCash(withdrawAmount)) { // ATM 클래스의 hasSufficientCash 메소드가 필요합니다.
            ...
            // 출금 처리
            atm->dispenseCash(withdrawAmount); // 최소 개수 지폐 지급을 위한 메소드 필요
...
int main() {
    ...
    //Account userAccount("123444", "K", "3333111122223333", 10000, myBank, "password");
    Interface ui;
